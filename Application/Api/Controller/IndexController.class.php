...
//                exit($this->http_url);
        ...
        //获取大分类
        $category_arr = M('goods_category')->where('parent_id=0')->field('id,name')->limit(3)->cache(true,TPSHOP_CACHE_TIME)->select();
        $result = array();
        foreach($category_arr as $c){
            ...
            //$goods = M()->query($sql);
            $goodsList = M('goods')->where("1=1")->limit(4)->cache(true,TPSHOP_CACHE_TIME)->getField("goods_id,goods_name,original_img,shop_price");
